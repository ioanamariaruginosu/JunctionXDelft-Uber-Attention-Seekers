...
class _DashboardPageState extends State<DashboardPage> with TickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          SlideToGoOnline(
            isOnline: session.activeSession,
            onChanged: (goOnline) async {
              if (goOnline) {
                await context.read<NotificationService>().startRestSession();
                context.read<MockDataService>().goOnline();
              } else {
                await context.read<NotificationService>().stopRestSession();
  ...
  Widget _buildMapInterface() => const RealMapWidget();

  Widget _buildTopBar(
      BuildContext context, AuthService authService, MockDataService mockData) {
  ...
  Widget _buildTripRequestCard(BuildContext context, MockDataService mockData,
    ...
    return Positioned(
      ...
      left: 16,
      right: 16,
      bottom: session.activeSession ? 100 : 160,
